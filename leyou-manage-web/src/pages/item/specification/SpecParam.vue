    ...
    <div>
        <v-data-table
            ...
            <template slot="items" slot-scope="props">
                ...
                <td class="text-xs-center">{{ formatBoolean(props.item.numtype) }}</td>
            ...
            <v-card>
                <v-card-text style="height: 300px;">
                    ...
                    <v-checkbox label="是否为数值类型" v-model="param.numtype" color="primary" hide-details/>
                    <v-text-field label="数值单位：" v-model="param.unit" v-if="param.numtype"/>
                    <v-checkbox label="是否用于搜索" v-model="param.searching" color="primary" hide-details/>
                    <v-flex v-if="param.searching && param.numtype" class="px-2">
                        搜索过滤区间：
